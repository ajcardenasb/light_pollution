library(dada2)
#setwd("~/Documents/Bioinformatics_scripts/R_scripts/light_pollution_R/test_dada2/")
### ### ### ### ### 
### Read files  ### 
### ### ### ### ###  
cat("Reading files")
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(200,200),maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE) # initially tried 220,220 but sample D11 did not pass
head(out)
...
write.table(asv.final, "light_pollution_ASV_table.txt",  quote = FALSE)
write.table(track, "light_pollution_ASV_stats.txt", quote = FALSE)
